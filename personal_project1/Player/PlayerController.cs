...
public class PlayerController : MonoBehaviour
{
    ...
    //토끼와 대화를 하고 나면 다음으로 가는 문을 열 수 있게 함.
    public bool isNextOpen = false;

    private ForestEntrance_PlayerFirstTalk forestEntrance_PlayerFirstTalk;
    ...
    void Update()
    {
        ...
        if (Input.GetButtonDown("Jump")&&!animator.GetBool("isJumping"))
        {
            ...
            {
                // 어차피 여기에는 토끼만 있어서 토끼 엔피씨가 있을 경우로 설정함
                if (scanObject.name == "NPC1")
                {
                    PlaySoundEffect("TALK");
                    gameManager.Action(scanObject);

                    //토끼 엔피씨한테 말 걸면 다음으로 가는 isNextOpen 활성화
                    isNextOpen = true;
                }
            }
            else if(!forestEntrance_PlayerFirstTalk.isStartLine)
            {
                gameManager.Action(forestEntrance_PlayerFirstTalk.Object);
                PlaySoundEffect("TALK");
            }
